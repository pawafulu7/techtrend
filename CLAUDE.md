...
### Phase 1実装 - ダークモード・未読管理（2025年8月24日 - 最新）
- **ダークモード実装**: next-themesによる切り替え機能、localStorage永続化
- **未読管理システム**: 未読バッジ、フィルター機能、自動既読マーク
- **UI改善**: おすすめトグルボタンのデザイン改善
- **データベース変更**: ArticleViewテーブルにisRead/readAtフィールド追加
- **パフォーマンス**: localStorageキャッシュでちらつき防止
