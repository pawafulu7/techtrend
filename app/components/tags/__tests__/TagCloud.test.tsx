...
const renderTagCloud = async (props?: TagCloudTestProps) => {
  ...
  await waitFor(
    () => {
      ...
      expect(
        result.container.querySelector('.animate-pulse')
...
describe('TagCloud', () => {
  ...
  describe('基本的なレンダリング', () => {
    it('タグクラウドが正しく表示される', async () => {
      ...
      expect(
        screen.getByRole('button', { name: '全期間' })
      ).toBeInTheDocument();
  ...
  describe('API通信', () => {
    it('正しいパラメータでAPIを呼び出す', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          '/api/tags/cloud?period=30d&limit=30'
        );
      });
    });
    ...
    it('期間を変更するとAPIを再呼び出しする', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          '/api/tags/cloud?period=30d&limit=50'
        );
      });
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          '/api/tags/cloud?period=7d&limit=50'
        );
      });
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          '/api/tags/cloud?period=all&limit=50'
        );
      });
    });
  ...
  describe('タグの表示', () => {
    it('タグ数が0の場合にメッセージを表示する', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('タグが見つかりませんでした')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Props', () => {
    it('limitプロパティが適用される', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          '/api/tags/cloud?period=30d&limit=10'
        );
      });
    });
