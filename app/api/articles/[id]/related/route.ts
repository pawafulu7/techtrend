...
type ArticleWithRelations = NonNullable<
  Awaited<ReturnType<typeof articleDetailCache.getArticleWithRelations>>
>;
...
export async function GET(
  ...
  try {
    ...
    const targetArticle =
      await articleDetailCache.getArticleWithRelations(articleId);

    if (!targetArticle) {
      return NextResponse.json({ error: 'Article not found' }, { status: 404 });
    }

    const shouldTryEmbedding =
      query.algorithm === 'embedding' || query.algorithm === 'auto';
    ...
    if (shouldTryEmbedding) {
      ...
      if (vectorSearch.isEmbeddingServiceAvailable()) {
        try {
          ...
          if (results.length > 0) {
            const articles = mapSearchResultToRelatedArticle(
              results,
              targetArticle
            );
    ...
    const relatedArticlesRaw = await articleDetailCache.getRelatedArticles(
      articleId,
      tagIds
    );
