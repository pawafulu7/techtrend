...
-- 空コンテンツ除外用の部分インデックス
CREATE INDEX IF NOT EXISTS idx_article_content_not_empty
ON "Article" (id)
...
-- タイプ用複合インデックス
CREATE INDEX IF NOT EXISTS idx_article_type_published
ON "Article" ("articleType", "publishedAt" DESC)
WHERE "articleType" IS NOT NULL;

-- ユーザ固有データ取得用インデックス
CREATE INDEX IF NOT EXISTS idx_favorite_user_article
