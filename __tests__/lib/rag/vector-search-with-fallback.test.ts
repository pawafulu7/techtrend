...
describe('VectorSearchService.searchWithFallback', () => {
  ...
  describe('enableFallback=false', () => {
    it('should return results without fallback', async () => {
      ...
      mockEmbeddingService.embedText.mockResolvedValue(new Array(1536).fill(0.5));
      mockPrisma.$queryRaw.mockResolvedValue(mockResults);

      const { results, metadata } = await service.searchWithFallback('Test Query', {
  ...
  describe('enableFallback=true', () => {
    it('should succeed at first threshold when enough results', async () => {
      ...
      const { results, metadata } = await service.searchWithFallback('Popular Topic', {
        enableFallback: true,
      });
    ...
    it('should fallback to lower threshold when initial fails', async () => {
      ...
      const { results, metadata } = await service.searchWithFallback('Niche Topic', {
        enableFallback: true,
      });
    ...
    it('should return results at 0.35 when all thresholds fail', async () => {
      ...
      const { results, metadata } = await service.searchWithFallback('Very Niche Topic', {
        enableFallback: true,
      });
