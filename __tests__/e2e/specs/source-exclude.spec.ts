...
test.describe('ソースフィルタリング機能', () => {
  ...
  test('複数ソースの選択状態を管理できる', async ({ page, browserName }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const sourcesBefore = new Set(
        (paramsBefore.get('sources') || '')
          .split(',')
          .filter(Boolean)
      );
      ...
      const sourcesAfter = new Set(
        (paramsAfter.get('sources') || '')
          .split(',')
          .filter(Boolean)
      );

      let removedSourceId = [...sourcesBefore].find((id) => !sourcesAfter.has(id));

      if (!removedSourceId) {
        const testId = await sourceCheckbox.getAttribute('data-testid');
        const fallbackId = testId?.replace('source-checkbox-', '');
        if (fallbackId && !sourcesAfter.has(fallbackId)) {
          removedSourceId = fallbackId;
        }
      }

      if (removedSourceId) {
        sourcesToDeselect.push(removedSourceId);
      }
    }
