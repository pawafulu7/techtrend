/**
 * Health Snapshot Calculation Batch Script
 *
 * Calculates health snapshots for all tech entities with mentions,
 * then invalidates related Redis caches.
 *
 * Usage:
 *   npx tsx scripts/scheduled/calculate-health-snapshots.ts
 *
 * Exit codes:
...
const PROCESS_NAME = 'health-snapshot-calculation';
...
async function calculateHealthSnapshots(): Promise<{
  calculated: number;
  errors: number;
}> {
  ...
  console.log(`[${PROCESS_NAME}] Starting health snapshot calculation...`);
  ...
  try {
    ...
    // Step 2: Calculate all health snapshots
    console.log(`[${PROCESS_NAME}] Step 2: Calculating health snapshots...`);
    const healthService = new TechHealthService(prisma);
    const result = await healthService.calculateAllHealth();

    console.log(
      `[${PROCESS_NAME}] Snapshots calculated: ${result.calculated} succeeded, ${result.errors} failed`
    );
...
export { calculateHealthSnapshots };
