...
async function generateJapaneseSummaries() {
  console.log('Dev.toとPublickeyの記事に日本語要約を生成します...\n');
  ...
  for (const source of sources) {
    ...
    // 英語の要約も対象に追加（Dev.toの場合）
    if (source.name === 'Dev.to') {
      const englishSummaryArticles = await prisma.article.findMany({
        where: {
          sourceId: source.id,
          NOT: {
            OR: [
      ...
      // 日本語が含まれていない要約をフィルタ
      const englishOnlyArticles = englishSummaryArticles.filter(article => 
        article.summary && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(article.summary)
      );
      
      articles.push(...englishOnlyArticles);
    }
