...
async function generateQualityReport() {
  ...
  try {
    ...
    console.log('📊 要約品質レポート');
    console.log('=' .repeat(80));
    
    console.log(`\n📈 全体統計:`);
    console.log(`   総記事数: ${metrics.totalArticles}件`);
    console.log(`   平均スコア: ${metrics.averageScore}/100`);
    
    console.log(`\n📊 スコア分布:`);
    console.log(`   優秀 (90-100): ${metrics.scoreDistribution.excellent}件 (${Math.round(metrics.scoreDistribution.excellent / metrics.totalArticles * 100)}%)`);
    console.log(`   良好 (80-89):  ${metrics.scoreDistribution.good}件 (${Math.round(metrics.scoreDistribution.good / metrics.totalArticles * 100)}%)`);
    console.log(`   普通 (70-79):  ${metrics.scoreDistribution.fair}件 (${Math.round(metrics.scoreDistribution.fair / metrics.totalArticles * 100)}%)`);
    console.log(`   要改善 (<70):  ${metrics.scoreDistribution.poor}件 (${Math.round(metrics.scoreDistribution.poor / metrics.totalArticles * 100)}%)`);
    
    console.log(`\n🔍 問題タイプ別集計:`);
    console.log(`   文字数問題:   ${metrics.issueTypes.length}件`);
    console.log(`   途切れ:       ${metrics.issueTypes.truncation}件`);
    console.log(`   内容薄い:     ${metrics.issueTypes.thinContent}件`);
    console.log(`   英語混入:     ${metrics.issueTypes.languageMix}件`);
    console.log(`   形式問題:     ${metrics.issueTypes.format}件`);
    
    console.log(`\n🔄 再生成統計:`);
    console.log(`   再生成必要:   ${metrics.regenerationStats.needed}件`);
    
    console.log(`\n📝 技術用語使用統計:`);
    console.log(`   総使用回数:   ${metrics.techTermsUsage.totalTermsUsed}回`);
    console.log(`   ユニーク用語: ${metrics.techTermsUsage.uniqueTermsUsed}種類`);
    
    if (metrics.techTermsUsage.topTerms.length > 0) {
      console.log(`\n   頻出技術用語TOP10:`);
      metrics.techTermsUsage.topTerms.forEach((item, index) => {
        console.log(`   ${index + 1}. ${item.term}: ${item.count}回`);
      });
    }
    ...
    sortedSources.forEach(([source, stats]) => {
      console.log(`   ${source}:`);
      console.log(`     - 記事数: ${stats.total}件`);
      console.log(`     - 平均スコア: ${Math.round(stats.averageScore)}/100`);
      console.log(`     - 問題あり: ${stats.issues}件 (${Math.round(stats.issues / stats.total * 100)}%)`);
    });
    ...
    if (metrics.scoreDistribution.poor > metrics.totalArticles * 0.1) {
      console.log('⚠️  10%以上の記事が低品質です。要約生成ロジックの見直しを推奨します。');
    }
    
    if (metrics.issueTypes.truncation > metrics.totalArticles * 0.05) {
      console.log('⚠️  5%以上の記事で途切れが発生しています。APIレスポンス処理の確認を推奨します。');
    }
    
    if (metrics.issueTypes.languageMix > metrics.totalArticles * 0.02) {
      console.log('⚠️  英語混入が多く検出されています。プロンプトの日本語指示を強化してください。');
    }
    
    if (metrics.averageScore < 85) {
      console.log('⚠️  平均スコアが目標の85点を下回っています。品質改善が必要です。');
    } else if (metrics.averageScore >= 90) {
      console.log('✨ 優秀な品質を維持しています！');
    }
