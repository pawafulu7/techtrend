...
async function testImprovedLocalLLM() {
  console.error('🧪 改善されたLocal LLMのテスト\n');
  console.error('デフォルトのmaxTokens: 800（環境変数未設定の場合）\n');
...
Server Actionsはフォーム処理のレイテンシを50%改善することが報告されています。
  ...
  try {
    ...
    console.error('\n📊 品質チェック:');
    console.error(`  ✅ 日本語で書かれている: ${hasJapanese ? 'OK' : 'NG'}`);
    console.error(`  ✅ 英語の思考過程が含まれない: ${!hasEnglishThinking ? 'OK' : 'NG'}`);
    console.error(`  ✅ 句点で終わる: ${endsWithPeriod ? 'OK' : 'NG'}`);
    console.error(`  ✅ 60-100文字の範囲内: ${lengthInRange ? 'OK (' + summary.length + '文字)' : 'NG (' + summary.length + '文字)'}`);
  ...
  console.error('\n' + '=' * 60);
  console.error('🏷️ 要約とタグ生成テスト（改善版）');
  console.error('=' * 60);
  ...
  try {
    ...
    console.error(`  ✅ 英語の思考過程が含まれない: ${!hasEnglishThinking ? 'OK' : 'NG'}`);
    console.error(`  ✅ 句点で終わる: ${endsWithPeriod ? 'OK' : 'NG'}`);
    console.error(`  ✅ 60-100文字の範囲内: ${lengthInRange ? 'OK (' + result.summary.length + '文字)' : 'NG (' + result.summary.length + '文字)'}`);
    console.error(`  ✅ タグが3-5個: ${hasValidTags ? 'OK (' + result.tags.length + '個)' : 'NG (' + result.tags.length + '個)'}`);
  ...
  console.error('\n' + '=' * 60);
  console.error('📈 改善効果のまとめ');
  console.error('=' * 60);
  console.error('1. maxTokensを800に増加 → 要約が途切れない');
  console.error('2. プロンプト改善 → 思考過程の出力を抑制');
  console.error('3. 環境変数対応 → LOCAL_LLM_MAX_TOKENSで調整可能');
}
