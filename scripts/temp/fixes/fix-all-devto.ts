...
8. 詳細要約は必ず6項目以上、第1項目は「記事の主題は」で始める
        ...
        if (isJapanese && hasGoodLength && notGeneric && hasProperTechnicalBackground && hasEnoughItems) {
          ...
          console.log(`  ✅ 成功: ${cleanedSummary.substring(0, 60)}...`);
          successCount++;
        } else {
          const problems = [];
          ...
          console.log(`  ⚠️ 失敗: ${problems.join(', ')}`);
          errorCount++;
        }
    ...
    console.log(`🚀 処理速度: ${((successCount + skipCount) / (totalTime / 60)).toFixed(1)}件/分`);
