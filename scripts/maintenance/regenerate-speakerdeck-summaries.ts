...
async function regenerateSpeakerDeckSummaries() {
  try {
    ...
    console.error('\n=== Speaker Deck要約生成対象 ===');
    console.error(`要約なしでcontent有り: ${articlesWithContent.length}件`);
    console.error(`十分なcontent & 要約なし: ${targetArticles.length}件`);
    ...
    targetArticles.forEach((article, index) => {
      console.error(`${index + 1}. ${article.title.substring(0, 50)}... (${article.content?.length}文字)`);
    });
    ...
    console.error(`要約あり: ${updatedArticles.length} (${(updatedArticles.length/totalArticles*100).toFixed(1)}%)`);
    console.error(`詳細要約あり: ${updatedWithDetailedSummary} (${(updatedWithDetailedSummary/totalArticles*100).toFixed(1)}%)`);
    console.error(`要約なし: ${totalArticles - updatedArticles.length} (${((totalArticles - updatedArticles.length)/totalArticles*100).toFixed(1)}%)`);
