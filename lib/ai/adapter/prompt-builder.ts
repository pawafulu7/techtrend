...
【詳細要約の形式ルール】
IMPORTANT: 詳細要約とAI評価は別のセクションです。以下は詳細要約の形式ルールです。

1. 項目間は必ず改行で区切る（最重要）
   - 各項目の直前に改行を入れる
   - 同じ行に複数の項目を並べてはならない（違反）
   - 項目と項目の間には必ず改行（\n）を入れる
...
【AI評価の生成ルール】
IMPORTANT: 以下は詳細要約とは別のセクション「AI評価」の形式ルールです。
