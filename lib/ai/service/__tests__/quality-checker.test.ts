...
describe('SummaryQualityChecker', () => {
  ...
  describe('基本的な品質チェック', () => {
    it('should pass quality check for valid summary', () => {
      // SUMMARY_LENGTH.idealMin(150) 以上にすること。
      // 150文字未満だと「一覧要約が短め」の minor 指摘が入り issues が空にならない。
      const summary =
        'TypeScriptの型システムを詳しく解説し、型安全性を保ちながら柔軟なコードを書く方法を紹介する。プリミティブ型からジェネリクス、ユニオン型まで、実践的な例とともに段階的に理解を深めていく構成となっている。型定義の設計指針や、既存のJavaScriptコードへ段階的に型を導入する手順も具体例つきで扱う。';
