...
export class CloudflareBlogFetcher extends BaseFetcher {
  ...
  async fetch(): Promise<FetchResult> {
    ...
    try {
          ...
          if (content && content.length < 2000) {
            ...
            if (enricher) {
              try {
                ...
                logger.error(
                  { err: _error },
                  `[Cloudflare Blog] Enrichment failed for ${item.link}`
                );
                // エラー時は元のコンテンツを使用
              }
            }
      ...
      errors.push(
        new Error(
          `Failed to fetch Cloudflare Blog RSS feed: ${_error instanceof Error ? _error.message : String(_error)}`
        )
      );
    }
  ...
  private generateCloudflareTags(
    categories?: string[],
    title?: string
  ): string[] {
