  ...
  describe('isEventArticle', () => {
    ...
    describe('イベントキーワードの判定', () => {
      test('「登壇」を含むタイトルはイベント記事と判定される', () => {
        expect(
          testIsEventArticle('登壇のお知らせ', 'https://example.com/post')
        ).toBe(true);
      });

      test('「イベント」を含むタイトルはイベント記事と判定される', () => {
        expect(
          testIsEventArticle('技術イベント開催', 'https://example.com/post')
        ).toBe(true);
      });

      test('「セミナー」を含むタイトルはイベント記事と判定される', () => {
        expect(
          testIsEventArticle('無料セミナーのご案内', 'https://example.com/post')
        ).toBe(true);
      });
      ...
      test('「参加募集」を含むタイトルはイベント記事と判定される', () => {
        expect(
          testIsEventArticle('ハッカソン参加募集中', 'https://example.com/post')
        ).toBe(true);
      });
    ...
    describe('URLパターンの判定', () => {
      test('URLに/eventを含む場合はイベント記事と判定される', () => {
        expect(
          testIsEventArticle('技術記事', 'https://example.com/event/123')
        ).toBe(true);
      });

      test('URLに/seminarを含む場合はイベント記事と判定される', () => {
        expect(
          testIsEventArticle('技術記事', 'https://example.com/seminar/tech')
        ).toBe(true);
      });

      test('URLに/meetupを含む場合はイベント記事と判定される', () => {
        expect(
          testIsEventArticle('技術記事', 'https://example.com/meetup/2025')
        ).toBe(true);
      });

      test('URLに/conferenceを含む場合はイベント記事と判定される', () => {
        expect(
          testIsEventArticle('技術記事', 'https://example.com/conference/tech')
        ).toBe(true);
      });
    });
    ...
    describe('日付パターンの判定', () => {
      ...
      test('タイトルに日付形式があってもイベント記事と判定される', () => {
        expect(
          testIsEventArticle('2030/12/1 開催予定', 'https://example.com/post')
        ).toBe(true);
      });
    });
    ...
    describe('通常の技術記事', () => {
      ...
      test('チュートリアル記事は除外されない', () => {
        expect(
          testIsEventArticle(
            'Docker入門チュートリアル',
            'https://example.com/tutorial/docker'
          )
        ).toBe(false);
      });
    });
    ...
    describe('複合パターン', () => {
      test('イベントキーワードとURLパターンの両方を含む場合', () => {
        expect(
          testIsEventArticle(
            '技術セミナー開催',
            'https://example.com/event/seminar'
          )
        ).toBe(true);
      });
      ...
      test('イベントキーワードがあるが例外キーワードも含む場合', () => {
        expect(
          testIsEventArticle('セミナー参加レポート', 'https://example.com/post')
        ).toBe(false);
      });
    });
  });
