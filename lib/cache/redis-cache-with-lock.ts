...
export class RedisCacheWithLock extends RedisCacheWithFallback {
  ...
  async getOrSetWithLock<T>(
    ...
    if (cached !== null) {
      this.stats.hits++;
      console.log(`[RedisCacheWithLock] Cache hit for key: ${key}`);
      return cached;
    }

    this.stats.misses++;
    console.log(`[RedisCacheWithLock] Cache miss for key: ${key}`);
    ...
    if (!lockToken) {
      // ロック取得失敗 - 他のプロセスが処理中
      console.log(`[RedisCacheWithLock] Waiting for cache generation for key: ${key}`);
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (retryResult !== null) {
          console.log(`[RedisCacheWithLock] Cache became available for key: ${key}`);
          return retryResult;
        }
      ...
      // それでも取得できない場合は直接実行
      console.log(`[RedisCacheWithLock] Timeout waiting for cache, fetching directly for key: ${key}`);
    ...
    try {
      ...
      if (doubleCheck !== null) {
        console.log(`[RedisCacheWithLock] Cache hit on double check for key: ${key}`);
        return doubleCheck;
      }

      // データを取得してキャッシュに保存
      console.log(`[RedisCacheWithLock] Fetching data with lock for key: ${key}`);
