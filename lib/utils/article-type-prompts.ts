...
要約:
記事の核心的な内容を150-180文字で簡潔にまとめてください。
主要な技術・ツール・概念を明記し、具体的な数値や効果があれば含めてください。
必ず句点で終了してください。

詳細要約:
記事の重要ポイントを5つの箇条書きで、合計500-600文字でまとめてください。
各項目は必ず「・」で開始し、改行で区切ってください。

・主題と背景（100-120文字）: 記事が扱う主要テーマと技術的文脈
・核心的内容（100-120文字）: 最も重要な情報や発見
・具体的詳細（100-120文字）: 実装方法、手順、具体例など
・価値と効果（100-120文字）: メリット、改善効果、適用場面
・補足情報（100-120文字）: 制約、注意点、関連情報など

タグ:
記事に関連する技術名、ツール名、概念を5個まで（カンマ区切り）
`;
