...
【最重要ルール - 文字数厳守】
1. 要約は必ず180-200文字に収める（200文字を超えたら失格）
2. 冗長な表現を避け、技術的価値を凝縮して表現
3. 箇条書きには句点（。）を付けない

要約:
【絶対条件】180文字以上200文字以下。この範囲外は失格。
【書き方】
- 冒頭20文字で記事の核心を示す
- 中間部分で技術的特徴を説明
- 最後に実用的価値を示す
- 冗長な接続詞（また、さらに、そして等）は削除
- 「について解説」「を紹介」等の決まり文句は使わない
...
・【120-160文字】核心：記事の主題と解決する課題を端的に説明
・【120-160文字】背景：なぜこの技術/手法が必要なのか、従来の問題点
・【120-160文字】解決策：提案されている具体的な技術的アプローチ
・【120-160文字】実装：コード例、設定、アーキテクチャの要点
・【120-160文字】効果：期待される改善、メリット、適用シーン
