...
export function validateDetailedSummary(detailedSummary: string): {
  ...
  if (detailedSummary.length < 200) {
    warnings.push(
      `詳細要約が短い可能性があります（${detailedSummary.length}文字）`
    );
  }

  // 構造化されているかチェック（6項目あるか）
  const bulletPoints = detailedSummary
    .split('\n')
    .filter((line) => line.trim().startsWith('・'));
  if (bulletPoints.length < 3) {
    errors.push(
      `詳細要約の項目数が少なすぎます（${bulletPoints.length}項目）。最低3項目必要です`
    );
  } else if (bulletPoints.length < 6) {
    warnings.push(
      `詳細要約の項目数が推奨より少ないです（${bulletPoints.length}項目）。推奨は6項目です`
    );
  }
