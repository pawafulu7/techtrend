  ...
  async enrich(url: string): Promise<EnrichmentResult | null> {
    try {
      ...
      if (
        ...
        // README content
        const readme = $(
          '.markdown-body, .readme, article[itemprop="text"]'
        ).first();
        if (readme.length) {
        ...
        // GitHub blob/tree/discussions pages often lack og:image
      ...
      // arXiv specific extraction
        ...
        urlObj &&
        (urlObj.hostname === 'arxiv.org' || urlObj.hostname === 'www.arxiv.org')
      ) {
        // Abstract
      ...
      // サムネイルが取得済みの場合は、GenericEnricherが失敗してもサムネイルを保持
