...
export class AWSEnricher extends BaseContentEnricher {
  ...
  canHandle(url: string): boolean {
    try {
      ...
      // 許可されたAWSドメインのリスト
      const allowedHosts = ['aws.amazon.com', 'www.aws.amazon.com'];
    ...
    try {
      ...
      for (const selector of contentSelectors) {
        ...
        if (element.length > 0) {
          // 不要な要素を削除
          element
            .find('script, style, noscript, iframe, nav, header, footer')
            .remove();
      ...
      if (!content || content.length < minLength) {
        logger.error(
          { url, contentLength: content.length, minLength },
          '[AWS Enricher] Insufficient content found'
        );
        return null;
      }
